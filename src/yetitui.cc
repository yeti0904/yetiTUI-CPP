...
vector <box_element> eputs(uint16_t x, uint16_t y, const char str[]) {
	vector <box_element> ret;
	box_element  add;
	int j = x;
...
vector <box_element> ePrintBox(uint16_t x, uint16_t y, uint16_t w, uint16_t h, bool border) {
	vector <box_element> ret;
		...
		for (int j = x; j<=w+x; ++i) {
			if (i == y) {
				if (((j == x) || (j == x+w)) && ((i == y) || i == y+h)) {
					if (border) {
						ret.push_back(createElement(j, i, '+'));
					}
					else {
						ret.push_back(createElement(j, i, ' '));
					}
